...
CREATE TABLE photographers.devis (id BIGSERIAL NOT NULL, company INT DEFAULT NULL, category BIGINT DEFAULT NULL, duration BIGINT DEFAULT NULL, currency BIGINT DEFAULT NULL, title VARCHAR(255) NOT NULL, presentation TEXT NOT NULL, price FLOAT, directpay SMALLINT DEFAULT 0 NOT NULL, note FLOAT, prestations INT NOT NULL, active SMALLINT DEFAULT 1 NOT NULL, createdAt TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, updatedAt TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, PRIMARY KEY(id));
...
CREATE INDEX IDX_132507C8865F80C0 ON photographers.devis (duration);
CREATE INDEX IDX_132507C86956883F ON photographers.devis (currency);
CREATE TABLE photographers.book (id BIGSERIAL NOT NULL, PRIMARY KEY(id));
...
CREATE TABLE prestations.prestation (id BIGINT NOT NULL, devis BIGINT DEFAULT NULL, client INT DEFAULT NULL, town BIGINT DEFAULT NULL, duration BIGINT DEFAULT NULL, status BIGINT DEFAULT NULL, reference VARCHAR(50) NOT NULL, price FLOAT, address VARCHAR(255) NOT NULL, start_time TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, createdAt TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, updatedAt TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, PRIMARY KEY(id));
...
ALTER TABLE photographers.devis ADD CONSTRAINT FK_132507C8865F80C0 FOREIGN KEY (duration) REFERENCES utils.duration (id) NOT DEFERRABLE INITIALLY IMMEDIATE;
